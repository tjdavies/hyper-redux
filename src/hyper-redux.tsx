...
export function makeContextConsumer<T, U>(): React.Consumer<{
  state: T;
  actions: U;
}> {
...
> {
  ...
  render() {
    return (
      <appContext.Provider
        value={{ state: this.state.value, actions: this.props.actions }}
      >
...
export function app<T, U>(
  ...
  if (actions["initialise"]) {
    actions["initialise"]({}, actions);
  }
